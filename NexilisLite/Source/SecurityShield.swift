...
//
//  Moved here from the StreamShield framework so that the checks it runs can stand in front of
//  connecting and signing in, rather than beside them. Everything it needs is private to this file
//  apart from SecurityShield itself; the extensions it used to carry were dropped, since the ones
//  NexilisLite already has say the same thing.
...
public class SecurityShield: NSObject {
    ...
    /// Puts a security alert on the screen, whatever else is on it.
    ///
    /// These used to be handed to `UIApplication.shared.visibleViewController`, which is nil often
    /// enough to matter: during launch, between screens, and while anything else is being presented
    /// or dismissed. When it came back nil the alert simply never appeared - and since each one
    /// carries the `exit(...)` that stops the app in its own button, nothing happened at all: no
    /// warning to the user, and no stop either. A window of its own depends on nothing already
    /// being on screen, so the warning always arrives.
    static func present(_ alert: UIAlertController) {
        let show = {
            ...
            guard let scene = scenes.first(where: { $0.activationState == .foregroundActive })
                    ...
                    ?? scenes.first else {
                // No scene at all yet - the app has not finished waking up. Ask again shortly
    ...
    /// Builds a security alert that always has something to read on it.
    ///
    /// The wording comes from the pulled policy, and when the policy could not be pulled - which is
    /// exactly when a check is most likely to trip - those come back empty, leaving a box with no
    /// text in it and nothing telling the user why the app is stopping.
    ...
    /// Held while their alert is up - a window with nothing keeping it alive goes away and takes
    /// the alert with it. Released again from `SSLibAlertController.viewDidDisappear`.
    ...
    public static func check(appName: String, apiKey: String) {
        // Only overwritten when there is something to write. Signing in can ask for the checks
        // before the app name and key have been settled, and blanking what is already stored would
        // send the policy request off without an account to ask about.
        if !appName.isEmpty {
...
//                }
    ...
    private static func pull() {
        ...
        tmessage.mCode = "SS01"
        tmessage.mStatus = CoreMessage_TMessageUtilSS.getTID()
        tmessage.mPIN = me
        ...
        DispatchQueue.global().async{
            postDataWithCookiesAndUserAgent(from: URL(string: Preference.getDomainOpr() + "get_feature_access_new")!) { data, response, error in
                ...
                if response?.statusCode != 200 || error != nil {
                    // Fix: this used to give up here. That was harmless while nothing waited on the
                    // outcome, but connecting and signing in wait on it now, and a server that
                    // cannot be reached would leave them waiting for ever. The checks still run -
                    // against the rules last received, which is what an empty answer already means
                    // below - so a network hiccup costs nothing and lets nothing through either.
    ...
    static func subCheck(_ typeSecurity : Int) {
        guard isRightState(code: typeSecurity) else {
            DispatchQueue.main.async {
                let alert = SecurityShield.alert(title: "", message: "Perangkat/jaringan/OS/aplikasi tidak memenuhi syarat keamanan.(" + checkErrorCode(code: typeSecurity) + ")")
...
//                print("ERROR 13")
    ...
    static func checkCallForward() -> Bool {
        if Preference.getCheckCallForward() && isCallForwarded() {
            DispatchQueue.main.async(execute: {
                let alert = SecurityShield.alert(title: Preference.getCheckCallForwardAlertTitle(), message: Preference.getCheckCallForwardAlertMessage())
    ...
    static func checkMultipleLogin() -> Bool {
        if Preference.getCheckMultipleLogin() && isMultipleLogin() {
            DispatchQueue.main.async(execute: {
                let alert = SecurityShield.alert(title: Preference.getCheckMultipleLoginAlertTitle(), message: Preference.getCheckMultipleLoginAlertMessage())
    ...
    static func checkSimSwap() -> Bool {
        if Preference.getCheckSimSwap() && isSimSwap() {
            DispatchQueue.main.async(execute: {
                let alert = SecurityShield.alert(title: Preference.getCheckSimSwapAlertTitle(), message: Preference.getCheckSimSwapAlertMessage())
                        ...
                        do {
                            if let jsonData = try JSONSerialization.jsonObject(with: stringData, options: []) as? [String: Any] {
                                ...
                                if result == 1 {
                                    ...
                                    DispatchQueue.main.async(execute: {
                                        let alert = SecurityShield.alert(title: Preference.getCheckClonedAlertTitle(), message: Preference.getCheckClonedAlertMessage())
        ...
        DispatchQueue.main.async {
            LocationFetcher.shared.getCurrentLocation { coordinate, score in
                if score > 0 {
                    ...
                    DispatchQueue.main.async(execute: {
                        let alert = SecurityShield.alert(title: Preference.getCheckGeoVelocityAlertTitle(), message: Preference.getCheckGeoVelocityAlertMessage())
...
//                        print("RESPON ANOMALI : \(responseString)")
                        if responseString == "ANOMALY_DETECTED" {
                            ...
                            DispatchQueue.main.async(execute: {
                                let alert = SecurityShield.alert(title: Preference.getCheckBehaviourAnalysisAlertTitle(), message: Preference.getCheckBehaviourAnalysisAlertMessage())
...
extension UIColor {
    static var mainColorSS: UIColor {
        return renderColorSS(hex: "#046cfc")
    }
    
    static var blackDarkModeSS: UIColor {
        return renderColorSS(hex: "#262626")
    }
    
    private class func renderColorSS(hex: String) -> UIColor {
...
private class CoreMessage_TMessageUtilSS {
...
private class SSLibAlertController: UIAlertController {
    /// Set when this alert was put up on a window of its own, so the window can go once the alert
    /// does. Alerts whose button ends the app never get here, and that is fine.
